...
from generation_utils import GenerationMixin # SentBS: using customized generation_utils
...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin, PushToHubMixin):
    ...
    def _load_state_dict_into_model(
        ...
        # MReD: extend positional embedding to allow longer source
        try:
            print("using transformers version 4.16.2")
            dim_modified_keys = []
            dim_modified_values = []
            init_state_dict = model.state_dict()
            init_state_dict_emb_pos = [key for key in init_state_dict.keys() if 'embed_positions' in key]
            state_dict_emb_pos = [key for key in state_dict.keys() if 'embed_positions' in key]
            init_size = -1
            preload_size = -1
            if len(init_state_dict_emb_pos) == len(state_dict_emb_pos):
                for (init_state_key, state_key) in zip(init_state_dict_emb_pos, state_dict_emb_pos):
                    init_size = init_state_dict[init_state_key].size(0)
                    preload_size = state_dict[state_key].size(0)
                    if preload_size < init_size:
                        print("extending ["+state_key+ "] using model initialized ["+init_state_key+"]")
                        print("from "+ str(preload_size) + " to "+ str(init_size))
                        dim_modified_keys.append(state_key)
                        init_val = init_state_dict[init_state_key] 
                        ## SentBS: extend untrained part using rand init
                        new_val = torch.cat((state_dict[state_key],init_val[preload_size:,:]),0)
                        dim_modified_values.append(new_val)
                    else:
                        print("using embed_position size:", preload_size)

            for key, val in zip(dim_modified_keys, dim_modified_values):
                state_dict[key] = val
        except: 
            print("error during counting embedding length, not extending length")
