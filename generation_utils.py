...
class GenerationMixin:
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        Examples:
            ...
            >>> print("Generated:", tokenizer.decode(outputs[0], skip_special_tokens=True))
        ```"""
        # SentBS: ADD self added model_kwargs
        gen_mode = model_kwargs.pop("gen_mode") if "gen_mode" in model_kwargs else None
        init_beam_scores = model_kwargs.pop("init_beam_scores") if "init_beam_scores" in model_kwargs else None
        ...
        if is_greedy_gen_mode:
            ...
            beam_scorer = BeamSearchScorer(
                ...
                # SentBS: ADD disallow stopping of beam search for sent generation
                gen_mode = gen_mode,
            )
            ...
            # SentBS: ADD pass the additional gen_mode
            if gen_mode is not None:
                model_kwargs["gen_mode"] = gen_mode
            if init_beam_scores is not None:
                model_kwargs["init_beam_scores"] = init_beam_scores
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        beam_scores[:, 1:] = -1e9 # this is to give the first beam a small advantage to allow the different beams to give different generations
        ...
        # SentBS:
        # receive gen_mode from model_kwargs
        gen_mode = model_kwargs.pop("gen_mode") if "gen_mode" in model_kwargs else None
        init_beam_scores = model_kwargs.pop("init_beam_scores") if "init_beam_scores" in model_kwargs else None
        ...
        while True:
            ...
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            # if past is used, only take the last input token for model_inputs["input_ids"]
            ...
            next_token_logits = outputs.logits[:, -1, :] # outputs.logits (batch_size, seq, vocab_size)
            ...
            next_token_scores_processed = logits_processor(input_ids, next_token_scores) # processed logprob of the current token generated
            ...
            next_token_scores = next_token_scores.view(batch_size, num_beams * vocab_size) # take topk out of all beam x new_token combi

            next_token_scores, next_tokens = torch.topk(
                next_token_scores, 2 * num_beams, dim=1, largest=True, sorted=True
            ) # (batch_size, 2*num_beams)

            next_indices = (next_tokens / vocab_size).long() # token_id
            next_tokens = next_tokens % vocab_size # beam_num_position
            ...
            beam_scores = beam_outputs["next_beam_scores"] # logsum probability of the sequence so far
            ...
            if return_dict_in_generate and output_scores:
                beam_indices = tuple((beam_indices[beam_idx[i]] + (beam_idx[i],) for i in range(len(beam_indices)))) # add the new beam_index to chains of beam_index for each sequence
        ...
        # after generation stop, beam_scorer will pad shorter beams with pad_token_ids and one eos_token_id
        sequence_outputs = beam_scorer.finalize(
            input_ids, # prev generated tokens
            beam_scores, # logsum prob of sequence
            next_tokens, # last token generated
            next_indices, # last beam idx of the token
            pad_token_id=pad_token_id,
            eos_token_id=eos_token_id,
            max_length=stopping_criteria.max_length,
        )
        ...
        if return_dict_in_generate:
            ...
            if self.config.is_encoder_decoder:
                return BeamSearchEncoderDecoderOutput(
                    ...
                    sequences_scores=sequence_outputs["sequence_scores"], # logsum / length**length_penalty
                    scores=scores, # logprob of each token_id
                    beam_indices=beam_indices, # batch_beam_index sequence for each token_id
                    encoder_attentions=encoder_attentions,
                    encoder_hidden_states=encoder_hidden_states,
                    decoder_attentions=decoder_attentions,
                    cross_attentions=cross_attentions,
    ...
    def beam_sample(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            next_token_scores = next_token_scores_processed + beam_scores[:, None].expand_as(next_token_scores) # logsum of the sequence
            next_token_scores = logits_warper(input_ids, next_token_scores)
            ...
            if return_dict_in_generate and output_scores: # chain of batch_beam_idx transitions for the corresponding token_ids
        ...
        sequence_outputs = beam_scorer.finalize(
            input_ids,
            beam_scores, # last token logprob
            next_tokens,
            next_indices,
            pad_token_id=pad_token_id,
            eos_token_id=eos_token_id,
